import { VideoProvider, getProviderById, VIDEO_PROVIDERS } from './video-providers'
...
interface RunwayMLRequestBody {
  ...
  keyframe_controls: boolean
  negative_prompt?: string
  image?: string
}

interface LumaRequestBody {
  prompt: string
  duration: number
  aspect_ratio: string
  model: string
...
interface OpenAISoraRequestBody {
  model: string
  prompt: string
  duration: number
  aspect_ratio: string
...
export class VideoProviderService {
  ...
  private async generateWithRunwayML(
    ...
    const requestBody: RunwayMLRequestBody = {
      prompt: request.prompt,
    ...
    if (request.conditioningImage) {
      requestBody.image = `data:${request.conditioningImage.mimeType};base64,${request.conditioningImage.imageBytes}`
    }
  ...
  private async generateWithLuma(
    ...
    const requestBody: LumaRequestBody = {
      prompt: request.prompt,
      duration: request.durationSeconds,
      aspect_ratio: request.aspectRatio,
  ...
  private async generateWithOpenAISora(
    ...
    const requestBody: OpenAISoraRequestBody = {
      model: 'sora-1.0',
      prompt: request.prompt,
      duration: request.durationSeconds,
      aspect_ratio: request.aspectRatio,
